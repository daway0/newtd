...
function isFormValid(requireds, nonRequireds, advanced) {
    ...
    error_toast("اشکال در اطلاعات فرم", "موارد ذکر شده در فرم را اصلاح کنید")
...
$(document).ready(function () {
    ...
    $(document).on('click', ".form-btn-container .form-btn", function () {
        ...
        // initiate select 2
        for (const id of select2Candidates) {
            if (mustBeSelect2.includes(getCleanElementId(id))) {
                $(`#${id}`).select2(select2Props)
            }
        }
    })
