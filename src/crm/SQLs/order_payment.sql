SELECT row_number() OVER (ORDER BY co.id) AS id,
    co.id AS order_id,
    COALESCE(services.order_cost, 0::numeric) AS cost_without_discount,
    COALESCE(services.order_cost - co.discount::numeric, 0::numeric) AS cost,
    COALESCE(services.order_healthcare_franchise_cost - co.discount::numeric, 0::numeric) AS healthcare_franchise,
    COALESCE(services.order_personnel_franchise_cost, 0::numeric) AS personnel_fee,
    ...
    COALESCE(services.order_personnel_franchise_cost - COALESCE(sum(personnel_payment.personnel_paid), 0::numeric), 0::numeric) AS personnel_debt,
    COALESCE(services.order_cost - co.discount::numeric - COALESCE(sum(client_payment.client_paid), 0::numeric), 0::numeric) AS client_debt
   FROM crm_order co
     LEFT JOIN ( SELECT co_1.order_id,
